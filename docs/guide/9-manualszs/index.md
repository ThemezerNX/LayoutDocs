...
- Fair knowledge on layout editing and diffing, see the available guides on the website
...
!!! tip
        The following tutorial will allow editing the `common.szs` files associated to `Psl.szs` (player selection menu) and `MyPage.szs` (user page), which by the way *aren't* part of `qlaunch`. This especially makes the bottom line removal possible for these applets, which is otherwise not possible by solely using Switch Theme Injector as it only supports the `common.szs` from `qlaunch`.
...
!!! warning
        **Now be careful, although we won't do anything major here, you don't want to accidentally mess up your NAND.**


| ![Goldleaf](goldleaf1.jpg "Goldleaf") | ![Goldleaf](goldleaf2.jpg "Goldleaf") |
| ------------------------------------- | --------------------------------------------- |
| `Manage console contents` | Looking for the title id |
...
| ------------------------------------- | --------------------------------------------- |
| Exporting `.nsp` | Confirm the `.nsp` export |
...
!!! info
        Don't pay attention to the `[WARN] prod.keys does not exist` error if you happen to get it.
...
| ------------------------------------- | --------------------------------------------- |
| Command line | `romfs` folder |

!!! tip
        Refer to this [repository](https://github.com/SciresM/hactool) for more info on using hactool.

### Diffpatching and installation
...
| ------------------------------------- | --------------------------------------------- |
| Themed Album applet | No more bottom line in player selection! |

### Implementing custom background images
